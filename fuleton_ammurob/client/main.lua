...
Citizen.CreateThread(function() 
    while ESX == nil do 
        ...
        TriggerEvent('fuleton_ammurob:blip')
        Citizen.Wait(0) 
    end 
end)
...
RegisterNetEvent('fuleton_ammurob:blip')
AddEventHandler('fuleton_ammurob:blip', function()
...
RegisterNetEvent('fuleton_ammurob:bliprobo')
AddEventHandler('fuleton_ammurob:bliprobo', function()
...
RegisterNetEvent('fuleton_ammurob:borrarblip')
AddEventHandler('fuleton_ammurob:borrarblip', function()
    RemoveBlip(bliprobo)
end)

RegisterNetEvent('fuleton_ammurob:disponible')
AddEventHandler('fuleton_ammurob:disponible', function()
    robable = true
    anim = true
    pedviva = true
end)

RegisterNetEvent('fuleton_ammurob:nodisponible')
AddEventHandler('fuleton_ammurob:nodisponible', function()
    robable = false
end)
...
Citizen.CreateThread(function()    
    while true do
        ...
        if anim and IsPedArmed(jugador, 5) and GetDistanceBetweenCoords(vector3(-332.04, 6085.28, 30.5), GetEntityCoords(jugador, true)) < 4 and robable then
            print('check')
            TriggerServerEvent('fuleton_ammurob:reloj')
            iniciado = true
            TriggerServerEvent('fuleton_ammurob:avisopoli')
            Citizen.Wait(10)
            ...
            if not IsPedDeadOrDying(ped) then
                ...
                TriggerServerEvent("fuleton_ammurob:fuerablip")
                TriggerServerEvent('fuleton_ammurob:reloj')
                iniciado = false
            end
            sleep = false
        ...
        while cajaentregada do
            Citizen.Wait(0)
            ESX.ShowHelpNotification("Pulsa ~INPUT_CONTEXT~ para revisar el contenido de la caja")
            ...
            if IsControlJustPressed(0, Keys["E"]) and GetDistanceBetweenCoords(vector3(-331.08, 6082.88, 31.44), GetEntityCoords(jugador, true)) < 2 then
                ...
                TriggerServerEvent("fuleton_ammurob:calle")
                DeleteObject(objeto)
                cajaentregada = false
                robable = false
            end   
        ...
        if iniciado then
            sleep = false
            DrawMissionText("Espera a que el empleado de la armmeria te entregue ~r~las armas")
        end
