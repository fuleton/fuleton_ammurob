...
RegisterServerEvent('fuleton_ammurob:calle')
AddEventHandler('fuleton_ammurob:calle', function()
    ...
    for i = 1, #xPlayers do
        ...
        if xPlayer.job.name == 'police' then
            TriggerClientEvent('fuleton_ammurob:borrarblip', xPlayers[i])
            TriggerClientEvent('esx:showNotification', xPlayers[i], 'Robo al ammunation terminado, el botín ha sido recogido')
        end
    end
end)

RegisterServerEvent('fuleton_ammurob:fuerablip')
AddEventHandler('fuleton_ammurob:fuerablip', function()
...
RegisterNetEvent('fuleton_ammurob:reloj')
AddEventHandler('fuleton_ammurob:reloj', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local xPlayers = ESX.GetPlayers()
    local reloj = 180
	...
	Citizen.CreateThread(function()
		while reloj > 0 do
			...
			if reloj > 0 then
                ...
                print(reloj)
                for i = 1, #xPlayers do
                    TriggerClientEvent('fuleton_ammurob:nodisponible', xPlayers[i])
                end
            end
            if reloj == 0 then
                print("Robodispo")
                for i = 1, #xPlayers do
                    TriggerClientEvent('fuleton_ammurob:disponible', xPlayers[i])
                end
            end
		end
    end)
...
RegisterServerEvent('fuleton_ammurob:avisopasma')
AddEventHandler('fuleton_ammurob:avisopasma', function()
    ...
    for i=1, #xPlayers, 1 do
        ...
        if xPlayer.job.name == 'police' then
            ...
            TriggerClientEvent('fuleton_ammurob:bliprobo', xPlayers[i])
        end
    end
end)
